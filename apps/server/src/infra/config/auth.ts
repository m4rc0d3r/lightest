...
const zAuthConfig = z
  .object({
    ...
    AUTH_ACCESS_TOKEN_LIFETIME: z.union([z.string().nonempty(), z.coerce.number().int()]),
    AUTH_REFRESH_TOKEN_SECRET: z.string().nonempty(),
    AUTH_REFRESH_TOKEN_LIFETIME: z.union([z.string().nonempty(), z.coerce.number().int()]),
...
function refine(
  obj:
    | Record<"AUTH_ACCESS_TOKEN_LIFETIME", string | number>
    | Record<"AUTH_REFRESH_TOKEN_LIFETIME", string | number>,
  ctx: z.RefinementCtx,
) {
